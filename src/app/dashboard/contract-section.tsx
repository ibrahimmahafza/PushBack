...
export default function ContractSection() {
  ...
  if (phase === "sparring-choose" && selectedClause) {
    return (
      <motion.div className="flex flex-col items-center py-16 px-6" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full max-w-lg">
          <button onClick={() => setPhase("sparring")} className="flex flex-col items-center gap-3 rounded-2xl border border-black/[0.06] bg-white/60 backdrop-blur-sm p-6 hover:shadow-md transition-all cursor-pointer text-center group">
          ...
          <button onClick={() => setPhase("sparring-voice")} className="flex flex-col items-center gap-3 rounded-2xl border border-black/[0.06] bg-white/60 backdrop-blur-sm p-6 hover:shadow-md transition-all cursor-pointer text-center group">
            <div className="flex h-14 w-14 items-center justify-center rounded-xl bg-green-100 group-hover:bg-green-200 transition-colors">
              <svg className="h-7 w-7 text-green-600" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M12 18.75a6 6 0 0 0 6-6v-1.5m-6 7.5a6 6 0 0 1-6-6v-1.5m6 7.5v3.75m-3.75 0h7.5M12 15.75a3 3 0 0 1-3-3V4.5a3 3 0 1 1 6 0v8.25a3 3 0 0 1-3 3Z" /></svg>
            </div>
        ...
        <button onClick={handleBackToAnalysis} className="mt-6 text-sm text-muted hover:text-foreground transition-colors cursor-pointer">Back to Analysis</button>
      </motion.div>
    );
  }
